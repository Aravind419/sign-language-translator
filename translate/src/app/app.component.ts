  ...
  updateToolbarColor() {
    ...
    const onColorSchemeChange = () => {
      ...
      if (!toolbar) {
        ...
        requestAnimationFrame(onColorSchemeChange);
        return;
      }
    ...
    window.addEventListener('app-theme-changed', onColorSchemeChange);
    // Also bind to mediaMatcher for system theme changes if needed
    const media = this.mediaMatcher.matchMedia('(prefers-color-scheme: dark)');
